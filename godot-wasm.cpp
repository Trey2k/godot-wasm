...
Variant Wasm::function(String name, Array args) {
  ...
  for (int i = 0; i < args.size(); i++) {
    ...
    case Variant::FLOAT:
      ...
      break;
    default:
      FAIL("Invalid argument type", NULL_VARIANT);
    }
  }
...
Variant Wasm::mem_read(uint32_t type, uint64_t offset, uint32_t length) {
  ...
  if (type == Variant::Type::INT) {
    ...
    for (uint32_t i = 0; i < length; i++)
      v.append((const int)mem_read(Variant::Type::INT, offset + i * sizeof(int64_t), length));
    return v;
  }
  else if (type == Variant::Type::PACKED_FLOAT64_ARRAY) {
    PackedFloat64Array v;
    for (uint32_t i = 0; i < length; i++)
      v.append((real_t)mem_read(Variant::Type::FLOAT, offset + i * sizeof(float64_t), length));
    return v;
  }
  return NULL_VARIANT;
}
